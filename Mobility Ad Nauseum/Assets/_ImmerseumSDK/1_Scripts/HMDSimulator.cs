    ...
    namespace VRSimulator {
        ...
        public class HMDSimulator : MonoBehaviour {
            ...
            public static bool isPlaystationVRHMDConnected {
                get {
                    if (VRDevice.isPresent == true && VRDevice.family == "PlayStationVR") {
                        return true;
                    }
                    return false;
                }
            ...
            void Start() {
                ...
                if (_isAcceptableCameraRigPresent) {
                    ...
                    Debug.LogError("[ImmerseumSDK] FATAL ERROR. Neither the SteamVR plugin ([CameraRig]) nor Oculus Rift (OVRCameraRig) cameras were found in your scene.");
            ...
            void OnInitializeHMDSimulator() {
                ...
                if (leftControllerPrefab == null && simulateControllers && controllerPrimitive == PrimitiveType.Custom) {
                    Debug.LogError("[ImmerseumSDK]/HMDSimulator: Controller Simulation is turned on using a Custom Controller, but Left Controller Prefab has not been set.");
                    throw new System.ArgumentNullException();
                } else if (rightControllerPrefab == null && simulateControllers && controllerPrimitive == PrimitiveType.Custom) {
                    Debug.LogError("[ImmerseumSDK]/HMDSimulator: Controller Simulation is turned on using a Custom Controller, but Right Controller Prefab has not been set.");
                    throw new System.ArgumentNullException();
                }
                ...
                if (isHMDConnected == false) {
                    if (CameraRig == null) {
                        if (Instance._packagesPresent == CameraRigPackages.Both || Instance._packagesPresent == CameraRigPackages.SteamVR) {
                            ...
                            Debug.LogError("[ImmerseumSDK] No supported CameraRig found / indicated.");
                    ...
                    if (isSteamVRHMDConnected) {
                        ...
                        Debug.LogError("Neither SteamVR nor Oculus HMD Connected");
                    }
                }
            ...
            void setSteamCameraRigInitialPosition(GameObject steamVRCamera) {
                ...
                if (isHMDConnected == false) {
                    ...
                    steamVRCameraContainer.transform.localPosition = new Vector3(cameraPosition.x, _standingAdjustment, cameraPosition.z);
            ...
            void setOculusCameraRigInitialPosition() {
                ...
                if (isHMDConnected == false) {
                    cameraPosition = new Vector3(cameraPosition.x, (cameraPosition.y - 1.06f), cameraPosition.z);
            ...
            void determineHandInputMode() {
                if (isSteamVRHMDConnected == true) {
                    ...
                    _isTrackingHands = false;
                    Debug.Log("[ImmerseumSDK]: No HMD connected, so no hand-tracking controllers active.");
                }
